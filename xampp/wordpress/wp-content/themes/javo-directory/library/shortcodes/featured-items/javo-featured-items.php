...
class javo_featured_items{
	...
	public function javo_featured_items_callback($atts, $content=""){
		wp_enqueue_style( 'javo-featured-items-css', JAVO_THEME_DIR.'/library/shortcodes/featured-items/javo-featured-items.css', '1.0' );
		...
		if( $javo_this_items->have_posts() ){
			while( $javo_this_items->have_posts() ){
				...
				$javo_this_results[$javo_this_results_key]['meta']	= sprintf('%s / %s ', $javo_meta_query->cat('item_location', 'No location'), $javo_meta_query->cat('item_category', 'No Category'));
