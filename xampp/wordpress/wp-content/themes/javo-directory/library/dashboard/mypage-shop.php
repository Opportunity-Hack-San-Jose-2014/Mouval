					...
					<div class="col-md-12">
						<div class="panel panel-default panel-wrap">
							...
							<!-- Starting Content -->
								...
								if( $javo_this_items->have_posts() ){
									while( $javo_this_items->have_posts() ){
										...
										if( $javo_tso->get('item_publish', '') == '' ){
											// Free
											$javo_paid_args['pay_state'] = 'glyphicon glyphicon-ok-circle';
											$javo_paid_args['pay_state_css'] = " active";
										}else{
											if(
												strtotime(date('YmdHis')) <=
												strtotime( get_post_meta($post->ID, "item_expired", true) )
											){
												// Actived
												$javo_paid_args['pay_state'] = 'glyphicon glyphicon-ok-circle';
												$javo_paid_args['pay_state_css'] = " active";
												...
												// Expired
												$javo_paid_args['pay_state'] = 'glyphicon glyphicon-remove-circle';
											...
											<div class="col-md-10">
												<div class="row">
													<div class="col-md-6 pull-left my-item-titles">
														<a href="<?php echo get_the_permalink($javo_this_parent_id);?>">
															<h3><?php the_title();?></h3>
															<span class="ratings">Ratings: <?php echo $javo_rating->parent_rating_average;?>/5</span>&nbsp;&nbsp;&nbsp;
															<span class="ratings">Reviews: <?php echo $javo_meta_query->get_child_count('review');?></span>
														</a>
													</div> <!-- col-md-6 -->

													<div class="col-md-6 pull-right">
														<div class="row content-panel-button-list" align="right">
															<a class="btn btn-success btn-circle<?php echo $javo_paid_args['pay_state_css'];?> mypage-tooltips" data-post="<?php the_ID();?>"  title="<?php _e('Payment Status', 'javo_fr'); ?>"><i class="<?php echo $javo_paid_args['pay_state'];?>"></i></a>
															<a href="<?php echo home_url('add-item/'.wp_get_current_user()->user_login.'/?edit='.get_the_ID());?>" type="button" class="btn btn-warning btn-circle mypage-tooltips" title="<?php _e('Edit Item', 'javo_fr'); ?>"><i class="glyphicon glyphicon-pencil"></i></a>
															<a class="btn btn-info btn-circle javo-this-publish<?php echo get_post_status(get_the_ID()) == 'publish'?' active':'';?> mypage-tooltips" data-post="<?php the_ID();?>" title="<?php _e('Show/Pause this Item', 'javo_fr'); ?>"><i class="glyphicon glyphicon-<?php echo get_post_status(get_the_ID()) == 'publish'?'pause':'play';?>"></i></a>
								...
								wp_reset_query();?>
								<?php get_template_part('templates/parts/part', 'item-active');?>
							<!-- End Content -->
							</div> <!-- panel-body -->
						</div> <!-- panel -->
					</div> <!-- col-md-12 -->
