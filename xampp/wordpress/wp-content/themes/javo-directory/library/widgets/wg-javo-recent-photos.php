		...
		<div class="widget_posts_wrap">
			...
			case "item":
				...
				foreach( $javo_recent_photos as $post ) : setup_postdata( $post ); ?>
					<li class="col-xs-4 col-sm-4 col-md-4">
						<?php if( $thumb == true ) : ?>
						<span class='thumb'><a href="<?php the_permalink(); ?>">
						<div class="img-wrap-shadow"><?php echo get_the_post_thumbnail($post->ID, "javo-tiny"); //get_avatar( get_the_author_meta('ID'), 32 ); ?></div>
						</a></span>
						<?php endif; ?>
					</li>
				<?php endforeach; wp_reset_postdata();
			...
			<ul class='latest-posts items list-unstyled'>
				...
				if( $javo_recent_photos->have_posts() ){
					while( $javo_recent_photos->have_posts() ){
						...
						$javo_this_permalink = '';
						switch( $post_type ){
							case 'jv_events': $javo_this_permalink = get_permalink( get_post_meta( get_the_ID(), 'parent_post_id', true)); break;
							case 'review': $javo_this_permalink = get_permalink( get_post_meta( get_the_ID(), 'parent_post_id', true)); break;
							...
							<span class='thumb'>
								<a href="<?php echo $javo_this_permalink; ?>">
									<div class="img-wrap-shadow">
										<?php the_post_thumbnail('javo-tiny');?>
									</div>
								</a>
							</span>
							<?php endif; ?>
