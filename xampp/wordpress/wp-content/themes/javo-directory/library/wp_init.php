<?php
	...
	function javo_wptuts_add_buttons( $plugin_array ) {
		$plugin_array['wptuts'] = get_template_directory_uri() . '/assets/js/javo-admin-editor.js';
		return $plugin_array;
	}
	...
	add_action("admin_init", "javo_add_lister_role");
	function javo_add_lister_role(){
	...
	function javo_login_redirect_callback($orgin, $req, $user){
		...
		return home_url('member/'. $user->user_login);
...
{
	global $submenu;
	unset($submenu['edit.php?post_type=lister'][10]);
}
function javo_hd_add_buttons() {
  global $pagenow;
  if(is_admin()){
	if($pagenow == 'edit.php' && (isset($_GET['post_type']) && $_GET['post_type'] == 'lister')){
...
function javo_drt_entry_meta() {
	...
	if ( $tag_list ) {
		$utility_text = __( 'Category : %1$s  Tags: %2$s Date : %3$s<span class="by-author"> by %4$s</span>.', 'javo_fr' );
	} elseif ( $categories_list ) {
		$utility_text = __( 'Category %1$s Date : %3$s<span class="by-author"> by %4$s</span>.', 'javo_fr' );
	} else {
		$utility_text = __( 'Date : %3$s<span class="by-author"> by %4$s</span>.', 'javo_fr' );
	}
...
function javo_new_user_callback($user_id) {
	...
	if(in_array('lister', $role_of_user)){
...
if ( '' == get_option( 'permalink_structure' ) ) {
    function wpse157069_permalink_warning() {
        ...
        echo '<p>We strongly recommend adding a <a href="' . esc_url( admin_url( 'options-permalink.php' ) ) . '">permalink structure (Post name)</a> to your site when using Javo Themes.</p>';
        echo '</div>';
    }
