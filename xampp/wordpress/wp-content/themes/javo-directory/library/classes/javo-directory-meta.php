...
class javo_get_meta{
	...
	public function get($meta_key, $default=NULL){
		...
		return !empty($this->javo_directory_meta[$meta_key])?$this->javo_directory_meta[$meta_key]:$default;
	}
	public function cat($taxonomy=NULL, $default=NULL, $single=TRUE, $return_array=FALSE){
		...
		if( !$return_array ){
			...
			$javo_this_return = $this->terms;
			if( $single ){
				$javo_this_return = $javo_this_return[0]->term_id;
			};
		};
