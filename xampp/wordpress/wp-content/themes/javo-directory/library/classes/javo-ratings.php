...
class javo_rating{
	...
	public function __construct($post_id=0){
		...
		add_action('save_post'						, Array($this, 'update_score'));
	...
	public function wp_enqueue_scripts_callback(){
		javo_get_style("../assets/css/timeline.css", "javo-rating-timeline-css", "1.0.0");
	}
										...
										<div class="row">
											<div class="col-md-4">
												<div class="rating-author">
													<?php echo wp_get_attachment_image( $javo_this_user_avatar_id, 'javo-tiny', true, Array('class'=>'img-circle'));?>
	...
	public function update_score($parent_post_id){
		...
		$parent_post_id = get_post_type($parent_post_id) == 'ratings' ? get_post_meta($parent_post_id, 'rating_parent_post_id', true) : $parent_post_id;
		...
		if( get_post_type($curent_rating_id) == 'ratings'){
			$javo_this_args['post__not_in'] = Array($curent_rating_id);
		}
		$javo_rating_posts = null;
		$javo_rating_posts = new wp_query($javo_this_args);

		if( $javo_rating_posts->have_posts() ){	
			while( $javo_rating_posts->have_posts() ){
				$javo_rating_posts->the_post();
				$this->total_average += (float) get_post_meta( get_the_ID(), 'rating_average', true);		
			}
			$this->average_score = (float)sprintf('%.1f', ($this->total_average / (int)$javo_rating_posts->found_posts));
		};
		wp_reset_postdata();
		update_post_meta( $parent_post_id, 'rating_average', 	$this->average_score);
		update_post_meta( $parent_post_id, 'rating_count',		$javo_rating_posts->found_posts);	
	}
	public function ajax_ratings_callback(){
		...
		$javo_rating_meta['average'] = (float)( $javo_rating_meta['summary'] / $javo_rating_meta['count'] );
		...
		if( !empty( $javo_rating_score ) ){
			foreach($javo_rating_score as $key => $value ){					
				...
				<div class="row">
					...
					<?php }; ?>
					<div class="<?php echo $show_title? 'col-md-6':'col-md-12';?> javo-tooltip" title="<?php echo $value['label'];?>" data-direction="left"	>
	...
	public function form($edit=0){
		...
		<form class="form-horizontal" role="form" id="javo_rating_form">
			...
			<div class="row">
				...
				<div class="col-md-6 rating-input-box">
					<?php
					...
					<div class="form-group">
						...
						<div class="row">
							<div class="col-md-12">
								<input id="javo_rating_submit" class="btn btn-dark col-md-12 admin-color-setting" value="<?php _e('Submit Rating', 'javo_fr');?>" type="button">
							</div>
						</div>
					</div>
			...
			<div class="col-md-12">
				...
				<div class="javo-rating-timeline-nofound hidden">
					<div class="alert alert-light-gray text-center">
						...
						<p><?php _e('Not found any ratings.', 'javo_fr');?>
					</div>
				</div>
			</div>
		</div>
								...
								<div class="row">
									<div class="col-md-5">						
										<div class="rating-author pull-left">
											<?php echo wp_get_attachment_image( get_the_author_meta('avatar'), 'javo-tiny', 1, Array('class'=> 'img-circle') ); ?>
											<div class="rating-total"><?php printf('%.1f', get_post_meta( get_the_ID(), 'rating_average', true ));?></div> <!-- rating-total -->
										</div> <!-- rating-author -->
									...
									<div class="rating-comments pull-left">
										<a href="<?php echo get_permalink($this->parent_post_id);?>#single-tab-ratings"><span><?php printf('%s %s', get_the_author_meta('first_name'), get_the_author_meta('last_name'));?> : </span>
											<?php
											if( (int)$length > 0 ){
