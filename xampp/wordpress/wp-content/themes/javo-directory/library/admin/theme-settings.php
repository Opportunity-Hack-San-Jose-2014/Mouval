...
class javo_theme_settings{
	...
	public function __construct(){
		...
		$pages = Array(
			...
			, "page"=> Array(__("item Pages", "javo_fr"), "<i class='fa fa-folder-open'></i>")
			, "itemfield"=> Array(__("item Fields", "javo_fr"), "<i class='fa fa-folder-open'></i>")
	...
	public function javo_theme_admin_enqueue(){
		...
		foreach($javo_enqueu_pages as $page=>$args){
			if(
				("theme-settings_page_javo-directory-settings-".$page == get_current_screen()-> id) ||
...
body{
	background-color:<?php echo $args->get('bg_color') ?>;
	opacity:<?php echo ($args->get('bg_color_opacity')/100) ?>
}
